...
StatusCode MCHierarchyAlgorithm::Run()
{
    ...
    IntVector mcId3D;
    FloatVector xx, yy, zz;
    for (const auto &[pMC, caloHits] : m_mcToHitsMap)
    {
        const LArMCParticle *const pLArMC{dynamic_cast<const LArMCParticle *>(pMC)};
        if (!pLArMC)
            continue;
        for (const CartesianVector &trajPoint : pLArMC->GetTrajPoints())
        {
            mcId3D.emplace_back(mcToIdMap.at(pMC));
            xx.emplace_back(trajPoint.GetX());
            yy.emplace_back(trajPoint.GetY());
            zz.emplace_back(trajPoint.GetZ());
        }
    }
    ...
    const int isCC{pLArMC ? pLArMC->GetIsCC() : false};
