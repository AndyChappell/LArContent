...
/*    std::set<const MCParticle *> foundLeading;
    for (const auto &[pMC, pLeading] : mcToLeadingMap)
    {
        if (foundLeading.find(pLeading) != foundLeading.end())
            continue;
        foundLeading.insert(pLeading);
        if (std::abs(pLeading->GetParticleId()) == E_MINUS)
        {
            this->Explore(pLeading);
        }
    }*/
    ...
    // Only non-EM and leading EM particles are in this list, which is why we don't need to do MC to leading checks
    for (const auto &[pMC, caloHits] : m_mcToHitsMap)
    {
    ...
    for (const auto &[pMC, caloHits] : m_mcToHitsMap)
    {
        // This should never heappen
        if (pMC != mcToLeadingMap.at(pMC))
        {
            std::cout << "Hierarchy problem" << std::endl;
            continue;
        }
        ...
        for (const MCParticle *const pChild : pMC->GetDaughterList())
            invisible += this->GetInvisibleContribution(pChild, nHitsU + nHitsV + nHitsW);
        const int pdg{std::abs(pMC->GetParticleId())};
        if ((pdg != 11 && pdg != 22) && ((nHitsU + nHitsV + nHitsW) > 9))
        {
            std::cout << "MC: " << pMC->GetParticleId() << " E: " << pMC->GetEnergy() << " p: " <<
                pMC->GetMomentum().GetMagnitude() << " Hits: " << (nHitsU + nHitsV + nHitsW) <<
                " invis: " << invisible;
        }
        const LArMCParticle *const pLArMC{dynamic_cast<const LArMCParticle *>(pMC)};
        float visibleEnergy{0.f};
        for (const auto &[pCurrent, pLeading] : mcToLeadingMap)
        {
            if (pLeading == pLArMC)
                visibleEnergy += pLArMC->GetVisibleEnergy();
        }
        if ((pdg != 11 && pdg != 22) && ((nHitsU + nHitsV + nHitsW) > 9))
            std::cout << " Visible: " << pLArMC->GetVisibleEnergy() << std::endl;
        ...
        PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_mcTreeName, "visible_energy", visibleEnergy));
        PANDORA_MONITORING_API(SetTreeVariable(this->GetPandora(), m_mcTreeName, "evis_fractrion", (visibleEnergy / pMC->GetEnergy())));
    ...
    const int isCC{(isNue || isNumu || isNutau) ? 1 : 0};
...
void MCHierarchyAlgorithm::Explore(const MCParticle *const pMC, std::string indent) const
{
    const CaloHitList &caloHitList{m_mcToHitsMap.find(pMC) != m_mcToHitsMap.end() ? m_mcToHitsMap.at(pMC) : CaloHitList()};
    const int pdg{pMC->GetParticleId()};
    const float energy{pMC->GetEnergy()};
    const float mom{pMC->GetMomentum().GetMagnitude()};
    const int nHits{static_cast<int>(caloHitList.size())};
    const int proc{dynamic_cast<const LArMCParticle *>(pMC)->GetProcess()};
    float adc{0.f};
    for (const CaloHit *const pCaloHit : caloHitList)
        adc += pCaloHit->GetInputEnergy();

    float invisible{0.f};
    for (const MCParticle *const pChild : pMC->GetDaughterList())
        invisible += this->GetInvisibleContribution(pChild, nHits > 0);

    std::cout << indent << pdg << ": E: " << energy << " p: " << mom << " Hits: " << nHits <<
        " ADC: " << adc << " Proc: " << proc << " Invis: " << invisible << std::endl;
    for (const MCParticle *const pChild : pMC->GetDaughterList())
        this->Explore(pChild, "  ");
}
...
float MCHierarchyAlgorithm::GetInvisibleContribution(const MCParticle *const pMC, const bool priorVisible) const
{
    const CaloHitList &caloHitList{m_mcToHitsMap.find(pMC) != m_mcToHitsMap.end() ? m_mcToHitsMap.at(pMC) : CaloHitList()};
    const float energy{pMC->GetEnergy()};
    const float mom{pMC->GetMomentum().GetMagnitude()};
    ...
    // If a parent was invisible, we've already subtracted off its energy, need to add some back
    // If the parent was visible, then we want to remove this particle's contribuion
    float contrib{0.f};
    if (priorVisible)
    {
        switch (proc)
        {
            ...
            case 6:
            case 13:
                contrib = nHits == 0 ? energy : 0;
                break;
            default:
                contrib = nHits == 0 ? mom : 0;
                break;
        }
    }
    else
