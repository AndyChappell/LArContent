...
StatusCode VertexSplittingAlgorithm::FindBestSplitPosition(const TwoDSlidingFitResult &slidingFitResult, CartesianVector &splitPosition) const
{
    ...
    if (!m_useSecondaryVertices && pVertexList->size() != 1)
        return STATUS_CODE_OUT_OF_RANGE;

    const Cluster *const pCluster(slidingFitResult.GetCluster());
    const HitType hitType(LArClusterHelper::GetClusterHitType(pCluster));
    float bestVertexDisplacementSquared{std::numeric_limits<float>::max()};
    CartesianVector bestSplitPosition(0, 0, 0);
    bool foundSplit(false);

    for (const Vertex *const pSelectedVertex : *pVertexList)
    {
        ...
        {
            foundSplit = true;
            if (vertexDisplacementSquared < bestVertexDisplacementSquared)
            {
                bestVertexDisplacementSquared = vertexDisplacementSquared;
                bestSplitPosition.SetValues(splitPosition.GetX(), splitPosition.GetY(), splitPosition.GetZ());
            }
        }
    }
    ...
    splitPosition.SetValues(bestSplitPosition.GetX(), bestSplitPosition.GetY(), bestSplitPosition.GetZ());
...
StatusCode VertexSplittingAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
    ...
    PANDORA_RETURN_RESULT_IF_AND_IF(STATUS_CODE_SUCCESS, STATUS_CODE_NOT_FOUND, !=, XmlHelper::ReadValue(xmlHandle, "UseSecondaryVertices", m_useSecondaryVertices));
