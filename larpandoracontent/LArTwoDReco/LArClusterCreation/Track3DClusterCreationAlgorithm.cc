/**
 *  @file   larpandoracontent/LArTwoDReco/LArClusterCreation/Track3DClusterCreationAlgorithm.cc
 *
...
Track3DClusterCreationAlgorithm::Track3DClusterCreationAlgorithm() :
...
StatusCode Track3DClusterCreationAlgorithm::Run()
{
...
StatusCode Track3DClusterCreationAlgorithm::FilterCaloHits(
...
StatusCode Track3DClusterCreationAlgorithm::AddFilteredCaloHits(
...
void Track3DClusterCreationAlgorithm::MakePrimaryAssociations(const OrderedCaloHitList &orderedCaloHitList,
...
void Track3DClusterCreationAlgorithm::MakeSecondaryAssociations(const OrderedCaloHitList &orderedCaloHitList,
    HitAssociationMap &forwardHitAssociationMap, HitAssociationMap &backwardHitAssociationMap) const
{
...
void Track3DClusterCreationAlgorithm::IdentifyJoins(const OrderedCaloHitList &orderedCaloHitList,
...
void Track3DClusterCreationAlgorithm::CreateClusters(
...
void Track3DClusterCreationAlgorithm::CreatePrimaryAssociation(const CaloHit *const pCaloHitI, const CaloHit *const pCaloHitJ,
    HitAssociationMap &forwardHitAssociationMap, HitAssociationMap &backwardHitAssociationMap) const
{
    ...
    // Apply equivalent check in associated views if present
    const LArCaloHit *const pLArCaloHitI{dynamic_cast<const LArCaloHit *>(pCaloHitI)};
    const LArCaloHit *const pLArCaloHitJ{dynamic_cast<const LArCaloHit *>(pCaloHitJ)};
    if (pLArCaloHitI && pLArCaloHitJ)
    {
        switch (pLArCaloHitI->GetHitType())
        {
            case TPC_VIEW_U:
            {
                const LArCaloHit *const pLArCaloHitIV{dynamic_cast<const LArCaloHit *>(pLArCaloHitI->GetSiblingHit(TPC_VIEW_V))};
                const LArCaloHit *const pLArCaloHitJV{dynamic_cast<const LArCaloHit *>(pLArCaloHitJ->GetSiblingHit(TPC_VIEW_V))};
                if (pLArCaloHitIV && pLArCaloHitJV)
                {
                    const float distanceSquaredV((pLArCaloHitJV->GetPositionVector() - pLArCaloHitIV->GetPositionVector()).GetMagnitudeSquared());
                    if (distanceSquaredV > m_maxCaloHitSeparationSquared)
                        return;
                }
                const LArCaloHit *const pLArCaloHitIW{dynamic_cast<const LArCaloHit *>(pLArCaloHitI->GetSiblingHit(TPC_VIEW_W))};
                const LArCaloHit *const pLArCaloHitJW{dynamic_cast<const LArCaloHit *>(pLArCaloHitJ->GetSiblingHit(TPC_VIEW_W))};
                if (pLArCaloHitIW && pLArCaloHitJW)
                {
                    const float distanceSquaredW((pLArCaloHitJW->GetPositionVector() - pLArCaloHitIW->GetPositionVector()).GetMagnitudeSquared());
                    if (distanceSquaredW > m_maxCaloHitSeparationSquared)
                        return;
                }
                break;
            }
            case TPC_VIEW_V:
            {
                const LArCaloHit *const pLArCaloHitIU{dynamic_cast<const LArCaloHit *>(pLArCaloHitI->GetSiblingHit(TPC_VIEW_U))};
                const LArCaloHit *const pLArCaloHitJU{dynamic_cast<const LArCaloHit *>(pLArCaloHitJ->GetSiblingHit(TPC_VIEW_U))};
                if (pLArCaloHitIU && pLArCaloHitJU)
                {
                    const float distanceSquaredU((pLArCaloHitJU->GetPositionVector() - pLArCaloHitIU->GetPositionVector()).GetMagnitudeSquared());
                    if (distanceSquaredU > m_maxCaloHitSeparationSquared)
                        return;
                }
                const LArCaloHit *const pLArCaloHitIW{dynamic_cast<const LArCaloHit *>(pLArCaloHitI->GetSiblingHit(TPC_VIEW_W))};
                const LArCaloHit *const pLArCaloHitJW{dynamic_cast<const LArCaloHit *>(pLArCaloHitJ->GetSiblingHit(TPC_VIEW_W))};
...
void Track3DClusterCreationAlgorithm::CreateSecondaryAssociation(const CaloHit *const pCaloHitI, const CaloHit *const pCaloHitJ,
    HitAssociationMap &forwardHitAssociationMap, HitAssociationMap &backwardHitAssociationMap) const
{
    HitAssociationMap::iterator forwardIter = forwardHitAssociationMap.find(pCaloHitI);
    HitAssociationMap::iterator backwardIter = backwardHitAssociationMap.find(pCaloHitJ);
...
const CaloHit *Track3DClusterCreationAlgorithm::GetJoinHit(
...
const CaloHit *Track3DClusterCreationAlgorithm::TraceHitAssociation(const CaloHit *const pCaloHit,
...
StatusCode Track3DClusterCreationAlgorithm::ReadSettings(const TiXmlHandle xmlHandle)
{
