...
private:
    ...
    pandora::FloatVector  m_hitScores;
    pandora::StringVector m_hitScoreLabels;
...
public:
    ...
    /**
     *  @brief  Write additional (LArCaloHit-specific) parameters into the FieldMap.
     */
...
inline unsigned int LArCaloHit::GetLArTPCVolumeId() const  { return m_larTPCVolumeId; }
inline unsigned int LArCaloHit::GetDaughterVolumeId() const { return m_daughterVolumeId; }
inline const pandora::FloatVector  &LArCaloHit::GetHitScores()      const { return m_hitScores; }
inline const pandora::StringVector &LArCaloHit::GetHitScoreLabels() const { return m_hitScoreLabels; }
...
inline float LArCaloHit::GetTrackProbability() const  { return m_pTrack.Get(); }
inline float LArCaloHit::GetShowerProbability() const { return m_pShower.Get(); }

//------------------------------------------------------------------------------------------------------------------------------------------

inline void LArCaloHit::SetTrackProbability(const float probability)
{
    if (probability >= 0.f) m_pTrack = probability;
    else throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);
}
...
{
    if (probability >= 0.f) m_pShower = probability;
    else throw pandora::StatusCodeException(pandora::STATUS_CODE_INVALID_PARAMETER);
}
