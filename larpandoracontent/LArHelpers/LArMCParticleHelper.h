...
public:
    ...
    /**
     *  @brief  Determine if the MC particle is a descendent of a particular particle
     *
     *  @param  mcParticle The descendent MC particle
     *  @param  pdg The PDG code to look for
     *
     *  @return True if mcParticle has an ancestor with code +-pdg, false otherwise.
     */
    static bool IsDescendentOf(const pandora::MCParticle *const mcParticle, const int pdg);

    /**
     *  @brief  Retrieves the unique ancestors of the MC particle.
     *
     *  @param  mcParticle The descendent MC particle
     *  @param  ancestors The vector to populate with ancestors
     */
    static void GetAncestors(const pandora::MCParticle *const mcParticle, std::vector<unsigned int>& ancestors);
