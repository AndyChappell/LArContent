...
bool LArMCParticleHelper::IsDescendentOf(const MCParticle *const mcParticle, const int pdg)
{
    const MCParticle* currentMCParticle{mcParticle};
    while (!currentMCParticle->GetParentList().empty())
    {   
        const MCParticleList& parentList{currentMCParticle->GetParentList()};
        if(parentList.size() > 1)
            throw StatusCodeException(STATUS_CODE_INVALID_PARAMETER);

        const MCParticle* const pParent{*parentList.begin()};
        const unsigned int parentPdg = std::abs(pParent->GetParticleId());
        if (parentPdg == pdg)
            return true;
        currentMCParticle = pParent;
    }
...
void LArMCParticleHelper::GetAncestors(const MCParticle *const mcParticle, std::vector<unsigned int>& ancestors)
{
    const MCParticle* currentMCParticle{mcParticle};
    while (!currentMCParticle->GetParentList().empty())
    {   
        ...
        const MCParticle* const pParent{*parentList.begin()};
        const unsigned int parentPdg = std::abs(pParent->GetParticleId());
        if (std::find(ancestors.begin(), ancestors.end(), parentPdg) == ancestors.end())
            ancestors.push_back(parentPdg);
        currentMCParticle = pParent;
    }
}
